...
static {
  ...
  private KBWedge mKbWedge = null; // initialize Wedge
  ...
  public void onCreate(Bundle icicle) {
    ...
    //KBWedge mKbWedge = null;

    String ACTION_USB_PERMISSION = "edu.sfsu.cs.orange.ocr.USB_PERMISSION";
  ...
  private void initOcrEngine(File storageRoot, String languageCode, String languageName) {
    ...
    if (ocrEngineModeName.equals("Both")) {
      ...
      indeterminateDialog.setMessage(
              "Initializing " + ocrEngineModeName + " OCR engine for " + languageName + "...");
    }
    ...
    new OcrInitAsyncTask(this, baseApi, dialog, indeterminateDialog, languageCode, languageName,
            ocrEngineMode).execute(storageRoot.toString());
  }
  ...
  boolean handleOcrDecode(OcrResult ocrResult) {
    ...
    if (isTranslationActive) {
      ...
      try {
        ...
        //if(mKbWedge == null)
        //{
        //  mKbWedge = new KBWedge((android.hardware.usb.UsbManager) getSystemService(Context.USB_SERVICE), this, ACTION_USB_PERMISSION);
        //  mKbWedge.Open();
        //}
        ...
        Matcher matcher = pattern.matcher(cityStateZip);
        if (matcher.find()) {

          // pattern matched!
          // extract and write city
          //writeText(outputStream, matcher.group(1).replaceAll(",", ""));
          //writeText(outputStream, "\t");
          ...
          //writeText(outputStream, "\t");
          mKbWedge.Write(matcher.group(2));
          mKbWedge.Write("\t");

          // extract and write zip
          //writeText(outputStream, matcher.group(3));
          //writeText(outputStream, "\t");
  ...
  //      cmd = "" + c;
  //    } else if ('A' <= c && c <= 'Z') {
  //      cmd = "left-shift " + Character.toLowerCase(c);
  //    } else {
  //      switch (c) {
  //        case '@':
  //          cmd = "left-shift 2";
  ...
  void handleOcrContinuousDecode(OcrResult ocrResult) {
    ...
    viewfinderView.addResultText(
            new OcrResultText(ocrResult.getText(), ocrResult.getWordConfidences(), ocrResult.getMeanConfidence(),
