 ...
 */
bool progressJavaCallback(int progress, int left, int right,
		int top, int bottom) {
...
jboolean Java_com_googlecode_tesseract_android_TessBaseAPI_nativeInitOem(JNIEnv *env, 
  ...
  tesseract::OcrEngineMode mode_tesseract;

  switch(mode) {
    case 0:
      mode_tesseract=tesseract::OEM_TESSERACT_ONLY;
      break;
    case 3:
      mode_tesseract=tesseract::OEM_DEFAULT;
      break;
    case 4:
      mode_tesseract=tesseract::OEM_LSTM_ONLY;
      break;
    case 5:
      mode_tesseract=tesseract::OEM_TESSERACT_LSTM_COMBINED;
      break;
      
  }

  if (nat->api.Init(c_dir, c_lang, mode_tesseract)) {
    LOGE("Could not initialize Tesseract API with language=%s!", c_lang);
    res = JNI_FALSE;
  } else {
    LOGI("Initialized Tesseract API with language=%s", c_lang);
